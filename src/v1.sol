...
contract KhoopDefiV2 is ReentrancyGuard {
    ...
    mapping(address => uint256) private _queueIndex;
...
function _processAvailableCycles() internal returns (uint256 totalCyclesProcessed) {
    ...
    // Add this to your KhoopDefiV2 contract
    function getPendingCyclesCount() public view returns (uint256) {
        uint256 balance = usdt.balanceOf(address(this));
        uint256 cyclesPossible = balance / CYCLE_PAYOUT;
        uint256 totalProcessable = 0;

        for (uint256 i = 0; i < entryQueue.length && cyclesPossible > 0; i++) {
            Entry storage entry = entries[entryQueue[i]];
            if (entry.isActive && entry.cyclesCompleted < MAX_CYCLES_PER_ENTRY) {
                uint256 remainingCycles = MAX_CYCLES_PER_ENTRY - entry.cyclesCompleted;
                uint256 cyclesToProcess = remainingCycles < cyclesPossible ? remainingCycles : cyclesPossible;
                totalProcessable += cyclesToProcess;
                cyclesPossible -= cyclesToProcess;
            }
        }

        return totalProcessable;
    }
